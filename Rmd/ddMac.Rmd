...
```{r, check1}
par=FLPar(c(m1=0.2/(300^-0.288),m2=-0.28,bmsy=c(refpts(eq)["msy","ssb"]),k=0.1,w50=par["a"]*par["l50"]^par["b"]))
...
```{r, check2}
save(om2,file="~/Desktop/tmp/t.RData")
load("~/Desktop/tmp/t.RData")
ggplot(model.frame(FLQuants(om2[,dim(m(om2))[2]],"Biomass"=biomass,"Yield"=catch)))+
  geom_line(aes(Biomass,Yield),col="red")+
  geom_line(aes(Biomass,Yield),data=model.frame(FLQuants(om[,dim(m(om))[2]],"Biomass"=biomass,"Yield"=catch)))
```
