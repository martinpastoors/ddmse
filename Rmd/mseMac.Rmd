...
```{r}
save(params,file="~/Desktop/tmp/parMac.RData")  
```
...
for (i in ac(2020:2080)) {
    ...
    prj[[3]] =ddFn(i,prj[[3]],params,massFlag=TRUE,matFlag=FALSE,mFlag=FALSE)
    control=as(FLQuants("f"=F[,i]),"fwdControl")
...
prjs=mlply(data.frame(F=rfpts$Fmsy), function(F) {
            ...
            for (iYr in ac(2020:2080)) {
              ...
              ## DD MM##################################################
              x3 =ddFn(iYr,x3,params,massFlag=TRUE,matFlag=TRUE,mFlag=TRUE)
