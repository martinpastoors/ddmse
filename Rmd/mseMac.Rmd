...
```{r}
save(om,eqM,rfpts,params,file="~/Desktop/tmp/t.RData")
```
...
```{r}
save(prjs,file="~/Desktop/tmp/t.RData")  
...
cast(ddply(smry,.(A,B), with, data.frame(F=median(F),SSB=median(SSB),Catch=median(Catch),F=median(F))),A~B,value="F")
