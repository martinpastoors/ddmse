...
```{r, sr}
load(paste("/home/laurie/Dropbox/pelagics/results/mac/figures/",tolower(stock),"_objects.RData",sep=""))
...
for (i in dimnames(vpaM)$year)
  om=ddFn(i,vpaM,par,FALSE,FALSE,TRUE)
```
...
prjs=mlply(data.frame(F=rfpts$Fmsy), function(F) {
            ...
            ## No DD ####################################################
            control=as(FLQuants("f"=F),"fwdControl")
            x1     =fwd(base,control=control,sr=eq,residuals=devRec)
            ...
            for (iYr in ac(2020:2080)) {
              ...
              x2 =fwd(x2,control=control,sr=eq,residuals=devRec)
                 
              ## DD MM##################################################
              x3 =ddFn(iYr,x3,par,massFlag=TRUE,matFlag=TRUE,mFlag=TRUE)
              control=as(FLQuants("f"=F[,iYr]),"fwdControl")
              x3 =fwd(x3,control=control,sr=eq,residuals=devRec)
  
              ## DD MMM ################################################
              x4 =ddFn(iYr,x4,par,massFlag=TRUE,matFlag=TRUE,mFlag=TRUE)
              control=as(FLQuants("f"=F[,iYr]),"fwdControl")
              x4 =fwd(x4,control=control,sr=eq,residuals=devRec)
              }  ## year loop
