...
weights are predicted from two variables: a smoother of total biomass/mean(total biomass) and age. The resulting factor is shown below. There is no clear relationship between biomass and weight-at-age.
...
The change in relative mass-at-age (i.e. observed - mean values) is plotted against total biomass, this allows the relationship between growth and total biomass to be compared across all ages. It is seen that there is no clear relationship between biomass and weight at age. A summary of the linear model is in the table below.

**???The trend of growth with biomass is -0.18, this was then used to model mass-at-age as:**
...
**Figure 20** summarizes the yields and SSBs obtained for each Operating Model under the correct and misspecified form of density dependence and the associated reference points, and **Figure 21** shows phase plots of the final 2050 distributions of biomass and yield. THese are for the Base Case, then density-dependence in mass, mass and maturity, and mass, maturity and natural mortality, projected for the four estimates of $F_{MSY}$. The correctly specified $F_{MSY}$ for each model is indicated by the red points and marginal densities.

The main diofferences are seen in $SSB/B_{MSY}$, if no density dependence is assumed then in the operating model the estimate of $F_{MSY}$ is the lowest, therefore projreting for the higher estimates of $F_{MSY}$ assuming density dependence results in an overfished stock. Yileld are the same for the two values of $F_{MSY}$ that assume no density-dependence on density dependence in mass-at-age only. In the other two cases yield is reduced but only by about 10%.
