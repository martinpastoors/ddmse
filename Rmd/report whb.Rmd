...
Estimating density-dependence from observations was not really possible for `r mystkname`. Therefore, density dependence in mass at age, maturity at age and natural mortality has only be explored from theoretical considerations in this report.
...
The change in relative mass-at-age (i.e. observed - mean values) is plotted against total biomass, this allows the relationship between growth and total biomass to be compared across all ages. Here, we plotted both the GAMM model and the Linear Model variants. It is seen that there is still some weak information on density dependence in the Linear Model (slope = beta = -0.11). A summary of the linear model is in the table below.
...
Estimating density-dependence from observations was only possible for mass-at-age but not maturity-at-age as the latter were assumed by ICES not to have varied over time. However, assuming that maturity is a function of mass-ata-ge allowed density deoendence to be modeeled. Natural mortality was assumed to be time and age invariant by ICES (i.e. 0.2), therefore density dependence in natural mortality can only be explored from theoretical considerations.
