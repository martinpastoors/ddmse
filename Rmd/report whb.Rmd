---
title: "Evaluating impacts of Density Dependent processes on Blue whiting via MSE simulations"
author:
- name: L.T. Kell
  affiliation: Sea++
- name: M.A. Pastoors
...
Version 1.1 (16 March 2023, fixed error in reference point table)

Version 1.2 (17 March 2023; fixed error in figure 15; added two scenarios)
...
The OMs were also projected for the estimates of different estimates $F_{MSY}$, with stochasticity in the recruitment deviates. This allows the impact of misspecification of density dependence to be evaluated, i.e. if $F_{MSY}$ is underestimated then yield will be less than MSY, while if $F_{MSY}$ is overestimated yield will be lost due to overfishing. Stochastic projections of the four Operating Models for the four estimates of $F_{MSY}$ are shown in **Figures 16-19**. **Figures 20-22** summarizes the yields, SSBs and Fs obtained for each Operating Model under the correct and mis-specified form of density dependence.

Estimating density-dependence from observations was not really possible for `r mystkname` because there was no clear relationship between mass-at-age and total biomass. When a second GAMM model was fitted with weight in relation to total biomass, year and age, we found that the mass-at-age is better described by a year effect than a biomass effect. In order to still explore the impacts of density dependence on the performance of the MSE, we used the density dependent factor for Atlantic mackerel (beta=-0.18) to parameterize the weight-biomass relationship. 

Including density dependence has a major impact on reference points. Especially $F_{MSY}$ is much higher when including DD. However, the expected yields in equilibrium conditions are expected to be very similar. The main difference is in the initial step from the current $F_{MSY}$ to any new $F_{MSY}$ that would include DD, as the higher $F_{MSY}$ would be applied to a stock that is estimated without DD.
...
Four alternative OMs have been included in the analyses:
...
Trends in (catch and stock) mass-at-age are shown in figure 1. Mean weight at ages 3-9 has generally decreased in the period 2010-2018, followed by an increase in the most recent years, for the most abundant ages in the catches. In 2022, a decrease in mean weight in almost all ages was observed.
...
The fixed maturity ogive for blue whiting was taken from the ICES assessment, this did not vary by year. Maturity-at-age was obtained by combining maturity ogives from the southern and northern areas, weighted by catch in numbers-at-age [@ICES1995BWWG, @ICES2022WHB].
...
Natural mortality is assumed to be 0.2 for all age groups and constant over time. The current M of 0.2 was derived from investigations undertaken in the 1980s that examined the age distribution of the stock before the industrial fishery started [@ICES1995BWWG].

During the WKPELA benchmark [@ICES2012WKPELA] various methods were explored to estimate how M may vary over ages. The relationship between natural mortality and body weight was applied to the blue whiting data to determine a variable M by age. The values ranged from around 1.1 at age 0 to 0.7 at age 10, which was considerably higher than the values used so far. During the benchmark, it was considered that there was no new information to support a revision of the used estimate of M (=0.2).
...
The variation in mass-at-age is shown in the figure below (left) and the relationship between mass-at-age and total biomass to the right. Consistent negative slopes on the biomass-mass-at-age plots are seen for ages 2-9.
...
**GAMM-at-age: biomass only**

A Generalized Additive Mixed Model (GAMM) was estimated to assess the relationship between mass-at-age and total biomass. Weights are predicted from two variables: a smoother of total biomass/mean(total biomass) and age. The resulting factor is shown below. There is no clear relationship between biomass and weight-at-age.
...
**GAMM-at-age: biomass and year smoothers**

Because no clear relationship between mass-at-age and total biomass was established, we fitted a second GAMM model with weight in relation to total biomass, year and age. This demonstrated that the mass-at-age is better described by the year effect than the biomass effect (lower p-value). 

In order to still explore the impacts of density dependence on the performance of the MSE, we used the density dependent factor for Atlantic mackerel (beta=-0.18) to parameterize the following relationship: 
...
fig_nums(
  name    = "cond_gammwaa2", level = 1, display = FALSE,
  caption = paste0(mystkname, ". GAMM analysis of mass-at-age against biomass, year and age"))

knitr::include_graphics(path=file.path(figuresdir, paste(section,mystk, "gamm2.jpg", sep="_")))

```

*`r fig_nums("cond_gammwaa2")`*

```{r, echo=FALSE, out.width = "400px", fig.align="center", message=FALSE, warning=FALSE, cache=FALSE}

tab_nums(
  name    = "cond_gammsummary2", level = 1, display = FALSE,
  caption = paste0(mystkname, ". GAMM summary of mass-at-age against biomass, year and age"))

cat(readLines(file.path(tablesdir, paste(section,mystk, "gamm2 summary.txt", sep="_"))), sep = '\n')

```

*`r tab_nums("cond_gammsummary2")`*
...
The distribution of maturity-at-age is shown in the figure below (left) and the relationship between maturity-at-age and total biomass is shown to the right. Obviously, as there is no time-dependent estimate of maturity, there is also no relationship with biomass.
...
Forward projections were run in two modes. Firstly in deterministic mode and secondly in probabilistic mode. We used the $F_{MSY}$ to project forward from 2020 until 2050. Reference points estimated for the different scenarios are in the table below. 
...
The comparison of the equilibrium curves, indicate that B_MSY is expected to decrease by 45%, $F_{MSY}$ is expected to increase by a factor of 5 and catch (MSY) is expected to increase by 8%. So, while inclusion of density-dependent processes has a substantial impact on biological reference points, the resulting change in equilibrium catch is expected to be small.
...
We used both the $F_{MSY}$ that is applicable to the density dependent scenario, but also evaluated what would happen if the 'wrong $F_{MSY}$' would be used with a particular scenario.
...
Using stochastic projections by including random processes for recruitment deviations and the $F_{MSY}$ values that are consistent with the different scenarios, leads to the overview below. SSB and total biomass are highest in the case of no density dependence. Biomass tends to be lower when density dependent processes are taken into account. Catches are different in the first years of 'switching' to the new reference points, but afterwards, the catches for the different scenarios are highly comparable.
...
For the stochastic projections at all $F_{MSY}$'s, we added two F scenarios: one with F=0.1 and one with F=`r myparams[myparams$stock==mystk,"highf"]`.

Misspecification of Fmsy, i.e. using an Fmsy from one scenario and applying it to a different scenario has only limited impacts on the overall outcomes in terms of recruitment, biomass or yield. In the plot below, the scenarios for density dependence are shown in the columns and the F's are shown as the colours. For example, a valid combination is the Base case with the yellow-brownish curve (Fmsy=`r format(rfpts[1,"Fmsy"], digits=2)`) or the M (mass) scenario with the green curve. The misspecification of Fmsy does not results in noticeable differences in catch although they do have important difference for biomass. Application of a low F (=0.1) is expected to lead to lower catches in all scenarios. 
...
For the four Fmsy-scenarios and the two additional F scenarios, we plotted the Kobe phase plot for 2050 with the associated density functions for B/Bmsy and Catch/MSY. The results confirm the results that equilibrium catch is not really affected by the DD assumptions of the Fmsy applied, but that biomass is sensitive to the assumptions. Application of density dependent reference points when density dependency would not exist, is expected to lead to lower equilibrium biomass levels.  

```{r, echo=FALSE, out.width = "300px", fig.align="center", message=FALSE, warning=FALSE, cache=FALSE}

fig_nums(
  name    = "kobe", level = 1, display = FALSE,
  caption = "Kobe plot for 2050 for base case (no DD) for all $F_{MSY}$'s (red is from the right scenario). Topleft: base, topright: M, bottomleft: MM, bottomright: MMM. White densities refer to the low and high F scenarios")
...
We then plotted the Kobe phase plot for 2050 with the associated density functions for F/Fmsy and Catch/MSY. Because there is no uncertainty included in the fishing mortality during the simulations, the marginal distributions of F do not exist in this plot. 

```{r, echo=FALSE, out.width = "300px", fig.align="center", message=FALSE, warning=FALSE, cache=FALSE}

fig_nums(
  name    = "kobe2", level = 1, display = FALSE,
  caption = "F Kobe plot for 2050 for base case (no DD) for all $F_{MSY}$'s (red is from the right scenario). Topleft: base, topright: M, bottomleft: MM, bottomright: MMM. White densities refer to the low and high F scenarios")

knitr::include_graphics(path=
                    c(file.path(figuresdir,paste("05",mystk,"kobe FY base.jpg",sep="_")),
                      file.path(figuresdir,paste("05",mystk,"kobe FY M.jpg",sep="_")),
                      file.path(figuresdir,paste("05",mystk,"kobe FY MM.jpg",sep="_")),
                      file.path(figuresdir,paste("05",mystk,"kobe FY MMM.jpg",sep="_"))
                      )
                    )
...
In the next step, we applied a natural mortality-at-age vector based on [@lorenzen2000allometry] which mimics the VPA assessment with fixed M-at-age. The figure below compares the SAM assessment with a Virtual Population Analysis (VPA) and a VPA with age varying M (VPAM). Trends and absolute levels were similar across all runs. The vpaM model was used as the base case to be compared against the density-dependent simulations.
...
2.  Next, VPA was conducted. This was done to ensure the MSE projections, which assumed variable M-at-age, and the historical estimates are comparable. The VPA was conducted on the whole catch matrix and the survivors in the most recent year and oldest age.
...
-   Base Case, VPA with varying M-at-age
-   Density dependence in mass-at-age,
-   Density dependence in mass, maturity-at-age,
-   Density dependence in mass, maturity and M-at-age,

The MSY reference points $B_{MSY}$ and $F_{MSY}$ were calculated based on equilibrium assumptions, either from combining yield per recruit and spawner per recruit relationship [@sissenwine1987alternative] or by conducting long-term projections under constant fishing moralities. Both should agree and so can be used as a check.
...
The OMs were also projected for the four estimates of  $F_{MSY}$, with stochasticity in the recruitment deviates. This allows the impact of misspecification of density dependence to be evaluated, i.e. if $F_{MSY}$ is underestimated then yield will be less than MSY, while if $F_{MSY}$ is overestimated yield will be lost due to overfishing. Stochastic projections of the four Operating Models for the four estimates of $F_{MSY}$ are shown in **Figures 16-19**

**Figure 20** summarizes the yields and SSBs obtained for each Operating Model under the correct and misspecified form of density dependence and the associated reference points, **Figure 21** shows the Kobe plots of the final 2050 distributions of biomass and yield and **Figure 22** the Kobe plots for F and yield. These are for the Base Case, then density-dependence in mass, mass and maturity, and mass, maturity and natural mortality, projected for the four estimates of $F_{MSY}$. The correctly specified $F_{MSY}$ for each model is indicated by the red points and marginal densities.

The main differences are seen in $SSB/B_{MSY}$, if no density dependence is assumed then in the operating model the estimate of $F_{MSY}$ is the lowest, therefore projecting for the higher estimates of $F_{MSY}$ assuming density dependence results in an overfished stock. Yield are the same for the two values of $F_{MSY}$ that assume no density-dependence on density dependence in mass-at-age only. In the other two cases yield is reduced by about 10%.

In the cases where density-dependence is assumed in the Operating Model, yields do not vary much, the main effect is seen in the estimates of $SSB/B_{MSY}$,
...
Since the early 1980s, natural mortality ($M$) for `r mystkname` has been fixed within the assessment model at 0.2, for all ages and all assessment years.Natural mortality must, however, vary both by age and over time due to changes in abundance of predators, feeding conditions, disease etc. [@siegfried2009review]. Particularly any changes in `r mystkname` distribution and growth will affect natural mortality.
