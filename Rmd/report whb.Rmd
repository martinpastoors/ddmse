...
**Laurie: why would DD in natural mortality have such a minimal impact? Did you see that the M levels overall are lower than the assumed M in the assessment (=0.2)**
...
In the next step, we applied a natural mortality-at-age vector based on [@lorenzen2000allometry] which mimics the VPA assessment with fixed M-at-age. The figure below compares the SAM assessment with a Virtual Population Analysis (VPA) and a VPA with age varying M (VPAM). Trends and absolute levels swere similar across all runs. The vpaM model was used as the base case to be compared against the density-dependent simulations.
...
The OMs were also projected for the four estimates of  $F_{MSY}$, with stochasticty in the recruitment deviates. This allows the impact of misspecification of density dependence to be evaluated, i.e. if $F_{MSY}$ is underestimated then yield will be less than MSY, while if $F_{MSY}$ is overestimated yield will be lost due to overfishing. Stochastic projections of the four Operating Models for the four estimates of $F_{MSY}$ are shown in **Figures 16-19**
