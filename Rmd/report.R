...
# for (mystk in c("mac","whb")) {
  ...
  load(file.path(dropboxdir,"data","om",paste0(mystk,".RData")))
  stk   = get(mystk)
  ...
  # simulate with DD for 100 years into the future (takes about 6 minutes)
  om2=om
  ptm <- proc.time()
  ...
  #             w50 = myparams[myparams$stock==mystk,"w50"])) 
  
  combY <- as.data.frame(pmYear(window(stk, start=2001)),drop=T) %>% mutate(stk=mystk, type="SAM non-DD") 
  
  # Density dependent SAM
  om_orig <- ddFn2(stk, par); 
  ...
  ggsave(p, 
         filename=file.path(figuresdir, paste(mystk, "om.jpg", sep="_")), 
         device="jpeg", 
         width=10, height=10, units="in")
