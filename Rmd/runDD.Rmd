...
subtitle: "Example of modelling density dependence in mass, maturity and M-at-age"
author: "L Kell"
...
Mackerel life history parameters are used to construct an example of modelling density dependence. An `FLBRP` object represents the equilibrium and an `FLStock` models the time series dynamics.
...
Where $\alpha$ and $\beta$ are taken from a linear regression of Mass~Total biomass, i.e. we model mass-at-age at each age based on empirical data.

$W_a$ is also constrained so sizes do not become unfeasibly small or large
...
Modelled as a logistic function of mass-at-age e.g.
